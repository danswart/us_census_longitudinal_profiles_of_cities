...
# =============================================================================
# PARAMETERS - CHANGE THESE FOR A DIFFERENT STATE/YEARS
# =============================================================================

params <- base::list(
  # NOTE: PUMS data is only available at state and PUMA level, not city level.
  # PUMAs are areas of ~100k+ people. For city-level analysis, you'd need to

  # identify which PUMA(s) contain your city and filter accordingly.
  state      = "TX",
  puma       = NULL,
  years      = 2019:2023,
  min_age    = 20L,
...
# =============================================================================
# To find PUMAs for a specific city, see:
# https://www.census.gov/programs-surveys/geography/guidance/geo-areas/pumas.html
# Or use tigris::pumas(state = "TX") to get PUMA boundaries
# =============================================================================
```
...
# -----------------------------------------------------------------------------
# PUMS data retrieval with caching
# -----------------------------------------------------------------------------

ds_get_pums_earnings <- function(state, puma = NULL, year, min_age = 20L,
                                 survey = "acs5", cache_dir) {
  key <- ds_cache_key("pums_earnings", base::list(
    state = state, puma = puma, year = year, min_age = min_age, survey = survey
  ))
  ...
  # PERNP = Total person earnings (available in some years)
  # PWGTP = Person weight
...
This document calculates earnings statistics for **adults `r params$min_age`+ with earnings** in **`r params$state`** using American Community Survey Public Use Microdata Sample (PUMS) data.

**Key difference from aggregate ACS tables:**

- PUMS microdata allows filtering to **exactly the population you want** (age 20+, with earnings)
- Weighted estimates account for survey design
...
**Important limitation:** PUMS data is only available at state and PUMA level, not city level. PUMAs are areas with ~100,000+ people. To analyze a specific city, you would need to identify which PUMA(s) contain that city.
...
ft_pce,
title = base::paste0(
  "Per Capita Earnings: ", params$state, " Adults ", params$min_age,
...
ggplot2::labs(
  title = base::paste0("Per Capita Earnings: ", params$state, " Adults ", params$min_age, "+"),
...
ft_mean,
title = base::paste0(
  "Mean Earnings: ", params$state, " Adults ", params$min_age,
...
ggplot2::labs(
  title = base::paste0("Mean Earnings: ", params$state, " Adults ", params$min_age, "+ with Earnings"),
  subtitle = base::paste0("PUMS ", params$survey, " (", params$base_year, " dollars)"),
  x = "Year",
  y = "Mean Earnings",
...
ft_median,
title = base::paste0(
  "Median Earnings: ", params$state, " Adults ", params$min_age,
  "+ with Earnings (", params$base_year, " dollars)"
)
)
ft_median
...
ggplot2::labs(
  title = base::paste0("Median Earnings: ", params$state, " Adults ", params$min_age, "+ with Earnings"),
  subtitle = base::paste0("PUMS ", params$survey, " (", params$base_year, " dollars)"),
  x = "Year",
  y = "Median Earnings",
  caption = "Shaded area represents standard error. Source: ACS PUMS"
...
ft_comp,
title = base::paste0(
  "Earnings Comparison: ", params$state, " Adults ", params$min_age,
...
To analyze a different state or age group, change these values in the `params` code chunk:
...
Clear cache when changing parameters:
...
- PUMS data is only available at **state** and **PUMA** level
- PUMAs are areas of ~100,000+ people
- City-level data is **not directly available** from PUMS
- To approximate a city, identify which PUMA(s) contain it:

```r
# Get PUMA boundaries for Texas
tx_pumas <- tigris::pumas(state = "TX", year = 2020)

# Find PUMA containing a city (requires city boundary shapefile)
```

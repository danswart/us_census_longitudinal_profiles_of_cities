...
ds_get_pums_gov_earnings <- function(city, state, puma_codes, year, min_age = 20L,
  ...
  key <- ds_cache_key("pums_gov_earnings_v3", base::list(
  ...
  if (!base::is.null(cached)) {
    base::message("Year ", year, " - Using cached data, cols: ", 
                  base::paste(base::names(cached), collapse = ", "))
    return(cached)
  }
  
  base::message("Year ", year, " - Fetching fresh data from Census API...")
  ...
  # Debug: show columns returned from tidycensus
  base::message("Year ", year, " - Columns from get_pums: ", 
                base::paste(base::names(raw), collapse = ", "))
 ...
 raw <- raw[raw$PUMA %in% puma_codes, ]
  ...
  # COW contains the numeric codes as characters
...
pums_list <- purrr::map(params$years, function(y) {
  tryCatch({
    result <- ds_get_pums_gov_earnings(
      city       = params$city,
      state      = params$state,
      puma_codes = puma_codes,
      year       = y,
    ...
    base::message("Year ", y, " returned ", base::nrow(result), " rows, cols: ",
                  base::paste(base::names(result), collapse = ", "))
    result
  }, error = function(e) {
...
base::message("Columns in pums_all: ", base::paste(base::names(pums_all), collapse = ", "))
base::message("Number of rows: ", base::nrow(pums_all))

# Check if we have data
if (base::nrow(pums_all) == 0L) {
  base::stop("No PUMS data retrieved. Check that the city/state/years are valid.")
}
```
